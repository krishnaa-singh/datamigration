...
def get_source_table_list():
    ...
    cur = conn.cursor()
    cur.execute("""SELECT  
           tablename,  (xpath('/row/cnt/text()', xml_count))[1]::text::int as record_count
    FROM (
      SELECT tablename, schemaname, 
             query_to_xml(format('SELECT count(*) AS cnt FROM %I', tablename), false, true, '') as xml_count
      FROM pg_catalog.pg_tables WHERE schemaname !~ '^pg_' AND schemaname <> 'information_schema'  --<< change here for the schema you want
    ) t 
    """)
    result = cur.fetchall()
    table_list = []
